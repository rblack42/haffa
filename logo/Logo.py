...
class Logo(object):
    ...
    def gen_raw_svg(self):
        ...
        for s in self.paths:
            ...
            svg += """ <path style="fill:none" stroke="black" stroke-width="3" d="%s" />""" % db
            svg += """
...
>""" % (width,height)
        ...
        for b in self.boxes:
            ...
            d = sp.d()
            svg += """
  <rect x="%d" y="%d" width="%d" height="%d" stroke="black" stroke-width="2" fill="none" />""" % (x1, y1, w, h)
            svg += """
  <path style="fill:none" stroke="red" stroke-width="3" d="%s" />""" % d
